...
- **4xx 系エラー**: クライアントエラー（認証失敗、不正リクエスト等）。再送しても結果が変わらないため削除する

ネットワークエラーと 5xx 系エラーは一時的な障害とみなし、削除せず再試行を継続します。
...
## 【6】Idempotency（べき等性）: 未実装

本章は将来の仕様です。現時点の実装では、べき等性キーの付与も重複判定も行いません。
キューの再送は、上流へ届いたかどうかを判別せずに実行するため、上流が処理を完了した直後に応答が届かなかった場合は再送により重複する可能性があります。重複排除が必要な場合は上流サーバ側で対応してください。
...
- **requestTimeout**: 60 秒（上流のヘッダ受信と本文受信にそれぞれ適用する制限時間）
